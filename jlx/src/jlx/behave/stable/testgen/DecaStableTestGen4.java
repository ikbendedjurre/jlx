...
public class DecaStableTestGen4 {
	...
	public DecaStableTestGen4(DecaStableFileReader file, float randomPathLengthMultiplier, boolean stateChangesOnly) {
		this.file = file;
		this.stateChangesOnly = stateChangesOnly;
...
//		System.out.println("maxSearchDepth = " + maxSearchDepth);
		...
		while (fringe.size() > 0 && depth <= maxSearchDepth) {
			newFringe.clear();
	...
	private int touchTransitions(Collection<Transition> transitions) {
		...
		for (Transition t : transitions) {
			if (!t.touched) {
				if (stateChangesOnly) {
					for (Transition t2 : t.getSrc().getOutgoing()) {
						if (t2.getTgt() == t.getTgt()) {
							touchTransition(t2);
							result++;
						}
					}
