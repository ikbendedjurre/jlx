...
public class ScenarioBreakdownAndRepairInTransit extends AbstractPendelTrainScenario {
	public ScenarioBreakdownAndRepairInTransit(PendelTrain_SR.Block mainDiagram) {
		super(mainDiagram);
	}
	...
	public Step[] getSteps() {
		return new Step[] {
			...
			expectStateChange(get_train(), "train[1][MOVING_1]"),
			expectStateChange(get_infrastructure(), "infrastructure[1][BREAKDOWN]", "infrastructure[3][TRAIN_AT_S1]"),
			...
			expectStateChange(get_infrastructure(), "infrastructure[3][TRAIN_AT_S1]", "infrastructure[2][TRAIN_AT_S2]"),
			...
			setInput(get_infrastructure().T5_repair, JPulse.TRUE),
			expectStateChange(get_train(), "train[3][MOVING_2]"),
			expectStateChange(get_infrastructure(), "infrastructure[4][BREAKDOWN]", "infrastructure[2][TRAIN_AT_S2]"),
			expectOutput(Output.from(get_infrastructure().D3_signal2, new SignalColor.RED()), Output.from(get_infrastructure().D3_signal2, new SignalColor.GREEN())),
			stabilize(),
			setInput(get_train().T1_arrival, JPulse.TRUE),
