 ...
 * Page 35
 */
public class F_P3_SR {
	public static class Block extends Type1IBD {
		public final PointPos Mem_last_Target_Requested = new PointPos(); //Missing in diagram!
		public final PointPos Mem_Current_Point_Position = new PointPos(); //Missing in diagram!
		...
		public final Initialization cOp1_Init = new Initialization(
			...
			"D6_Detection_State := \"Detection_State.FALSE\";", //Set to FALSE in diagram, which is wrong type! + Changed enum.
			...
			"D20_Point_Position := \"UNKNOWN\";", //Replaces line above.
			"D25_Redrive := FALSE;",
		...
		public final Operation<JBool> cOp2_All_Left = new Operation<>(
		...
		public final Operation<JBool> cOp3_No_End_Position = new Operation<>(
			"return not cOp6_Error() and not cOp5_Trailed() and not cOp2_All_Left() and not cOp4_All_Right();"
		);
		
		public final Operation<JBool> cOp4_All_Right = new Operation<>(
			"if cOp8_Supports_Multiple_PMs() then",
			"  return (",
		...
		public final Operation<JBool> cOp5_Trailed = new Operation<>(
			"if not cOp6_Error() then",
			"  if cOp8_Supports_Multiple_PMs() then",
			"    return D21_PM1_Position = \"PointPos.TRAILED\" or D22_PM2_Position = \"PointPos.TRAILED\";", //Line below gives more consistent input equivalence classes:
			//"    return D21_PM1_Position = \"PointPos.TRAILED\" or (D13_PM2_Activation = \"ACTIVE\" and D22_PM2_Position = \"PointPos.TRAILED\");",
			"  else",
			"    return D21_PM1_Position = \"PointPos.TRAILED\";",
			"  end if",
			"else",
			"  return FALSE;",
			"end if"
		...
		public final Operation<JBool> cOp6_Error = new Operation<>(
			"if cOp8_Supports_Multiple_PMs() then",
			"  return D21_PM1_Position = \"ERROR\" or D22_PM2_Position = \"ERROR\";", //Line below gives more consistent input equivalence classes:
			//"  return D21_PM1_Position = \"ERROR\" or (D13_PM2_Activation = \"ACTIVE\" and D22_PM2_Position = \"ERROR\");",
			"else",
			"  return D21_PM1_Position = \"ERROR\";",
			"end if"
		);
		
		public final Operation<JBool> cOp7_Is_Trailable = new Operation<>(
			"return D32_Con_007600 or D33_Con_007900 or D34_Con_008000 or D35_Con_008200;"
		);
		
		public final Operation<JBool> cOp8_Supports_Multiple_PMs = new Operation<>(
			"return D30_Con_007000 or D32_Con_007600 or D33_Con_007900 or D34_Con_008000 or D35_Con_008200 or D37_Con_008400;"
		);
		
		public final Operation<JBool> cOp9_Redrive_Enabled = new Operation<>(
			"return D30_Con_007000;"
		);
		
		public final Operation<JBool> cOp10_Redrive_Right = new Operation<>(
			"return cOp3_No_End_Position() and Mem_last_Target_Requested = \"RIGHT\" and Mem_Current_Point_Position = \"RIGHT\";"
		);
		
		public final Operation<JBool> cOp11_Redrive_Left = new Operation<>(
			"return cOp3_No_End_Position() and Mem_last_Target_Requested = \"LEFT\" and Mem_Current_Point_Position = \"LEFT\";"
		);
		
		public final Operation<JVoid> cOp12_Timeout = new Operation<>(
			"D5_Drive_State := \"STOPPED\";",
		...
		public final Operation<JBool> cOp13_Not_Initialised = new Operation<>(
		...
		public final OutPort<PointPos> D20_Point_Position = new OutPort<>();
