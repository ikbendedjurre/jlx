...
import jlx.behave.stable.DecaStableCoverage;
import jlx.behave.stable.DecaStableLevels;
import jlx.behave.stable.files.*;
import jlx.behave.stable.files.DecaStableFile.*;
import jlx.behave.stable.testgen.DecaStableTestGen4;
import jlx.behave.stable.testgen.DecaStableTestGen5;
import jlx.behave.stable.testgen.ExecutionTime;
import jlx.behave.verify.*;
import jlx.blocks.ibd1.AdapterLabels;
...
//		EfeS_SCI_XX_Protocol_Stack_SR.Block ps = m.add("ps", new EfeS_SCI_XX_Protocol_Stack_SR.Block()); //Any name is fine.
		...
		UnifyingBlock ub = new UnifyingBlock("point", m, true, true);
//		ub.stableSm.printGraphvizFile();
//		DecaStableCoverage cov = new DecaStableCoverage(ub.stableSm);
//		cov.saveToFile("point.sic.cov");
//		DecaStableFileWriter w = new DecaStableFileWriter();
		
//		new DecaStableFileBuilder(ub.stableSm).saveToFile("sic", "point.sic");
...
////		new MCRL2Printer(ub, new PrintingOptions()).printAndPop("models/point");
		...
		new DecaStableLevels(ub).saveToFile("levels.txt");
		
		System.out.println("#time-elapsed = " + (0.001f * (System.currentTimeMillis() - prevTime)) + "s");
	}
	
	public static void main888(String[] args) throws ReflectionException, IOException {
		DecaStableFileWriter r = new DecaStableFileWriter();
		r.loadFromFile("sic", "point.sic", false);
		
		ExecutionTime et = new ExecutionTime(10L, 20000L);
		et.add(r);
		et.print();
	}
	
	public static void main44(String[] args) throws ReflectionException, IOException {
		DecaStableFileWriter r = new DecaStableFileWriter();
		r.loadFromFile("sic", "point.bisim.sic", false);
		
//		new DecaStableTestGen4(r).populateTests(0f, true);
//		r.saveToFile("sic", "point.2.sic");
//		
//		DecaStableFileReader r2 = new DecaStableFileReader();
//		r2.loadFromFile("sic", "point.2.sic", false);
//		
//		for (int i = 0; i < r.getTests().size(); i++) {
//			if (r2.getTests().get(i).getTransitions().size() != r.getTests().get(i).getTransitions().size()) {
//				throw new Error("Test length mismatch!");
//			}
//		}
//		
//		System.exit(0);
		...
		r.saveToFile("sic", "point.bisim.t77.sic");
		
//		int id = 0;
//		
//		for (Trace test : r.getTests()) {
//			if (test.getInterestingness() > 10) {
//				System.out.println("TEST " + id);
//				
//				int i = 1;
//				int j = 1;
//				
//				for (Transition t : test.getTransitions()) {
//					System.out.println();
//					t.print("[" + j + "] ");
//					j++;
//					
...
//		}
//		
//		ExecutionTime et = new ExecutionTime(100L, 120000L);
//		et.add(r);
//		et.print();
	}
	
	public static void main99(String[] args) throws ReflectionException, IOException {
		DecaStableFileWriter r = new DecaStableFileWriter();
		r.loadFromFile("sic", "point.sic", false);
		
		Port T10_port = r.findInputPort("sp", "T10_Move_Point");
		Port T20_port = r.findOutputPort("sp", "T20_Point_Position");
		
//		for (OutputEvolution evo : r.getOutputEvolutions().values()) {
//			for (Map<Port, String> e : evo.getEvolution()) {
//				if (e.containsKey(T20_port)) {
//					System.out.println("Found T20!");
//					System.out.println("\t" + evo.toString());
//					
//					if (evo.getLast().containsKey(T20_port)) {
//						System.out.println("Also found at the end");
//					} else {
//						System.out.println("Not found at the end?!!");
//					}
//					
//					CLI.waitForEnter();
//				}
...
//		}
		...
		for (Transition t : r.getTransitions()) {
			String v = t.getInputChanges().getNewValuePerPort().get(T10_port);
			
			if (v != null) {
				for (OutputEvolution evo : t.getOutputEvolutions()) {
					if (evo.contains(T20_port, "\"TRUE\"")) {
	...
	public static void main999(String[] args) throws ReflectionException, IOException {
		DecaStableFileBisimReducer r = new DecaStableFileBisimReducer();
		r.loadFromFile("sic", "point.sic", false);
		
		DecaStableFileWriter dest = new DecaStableFileWriter();
		r.writeTo(dest);
		dest.saveToFile("sic", "point.bisim.sic");
		
		DecaStableFileReader x = new DecaStableFileReader();
		x.loadFromFile("sic", "point.bisim.sic", false);
	}
	
	public static void main7(String[] args) throws ReflectionException, IOException {
		DecaStableFileWriter r = new DecaStableFileWriter();
		r.loadFromFile("sic", "point.tsuite2.sic", false);
		
		ExecutionTime et = new ExecutionTime(10L, 120000L);
		et.add(r);
		et.print();
